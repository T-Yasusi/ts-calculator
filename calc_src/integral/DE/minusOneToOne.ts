...
export default function minusOneToOne(
    f: ((x: number) => number) | ((x: Complex) => Complex),
    threshold: number = 1.0e-3
){
    const isReal = typeof f === 'function' && f.length === 1 && typeof f(0 as any) === 'number';

    if( isReal ){
        const phi = (t: number ): number => tanh( (Math.PI/2)*sinh(t)) as number;                        
        const phiPrime = (t: number ): number => (Math.PI/2)*cosh(t)*
                                                 (1/cosh((Math.PI/2)*sinh(t)))**2 as number;
        const g = (t: number ): number => (f as (x: number)=> number)(phi(t))*phiPrime(t);

        return converge(g, threshold);
    }
    else {
        const phi = (t: Complex): Complex => tanh((Math.PI / 2) * sinh(t)) as Complex;
        const phiPrime = (t: Complex): Complex => (Math.PI / 2) * cosh(t) * 
                                                  (1/cosh((Math.PI / 2) * sinh(t)))**2 as Complex;
