...
export default function minusInfToInf(
    ...
    if( isReal ){
        const phi = (t: number ): number => exp( (Math.PI/2)*sinh(t) ) as number;                        
        const phiPrime = (t: number ): number => (Math.PI/2)*cosh(t)*phi(t) as number;
        const g = (t: number ): number => (f as (x: number)=> number)(phi(t))*phiPrime(t);

        return converge(g, threshold);
    }
    else {
        const phi = (t: Complex): Complex => exp((Math.PI / 2) * sinh(t)) as Complex;
        const phiPrime = (t: Complex): Complex => (Math.PI / 2) * cosh(t) * phi(t) as Complex;
