...
export default function (cofficients, thre = 10e-10) {
    const result = [];
    ...
    if (cofficients.length === 2) {
        if (cofficients[1] instanceof Complex)
            result.push(div(neg(cofficients[1]), cofficients[0]));
        else
            result.push(new Complex(div(neg(cofficients[1]), cofficients[0]), 0));
    }
...
function step(coffs, thre = 10e-10) {
    if (abs(coffs[0]) > 0)
        for (let i = sub(coffs.length, 1); i >= 0; i--)
            coffs[i] = div(coffs[i], coffs[0]);
    let p = 1;
    let q = 1;
    const b = [1, sub(coffs[1], p)];
    for (let i = 2; i < coffs.length; i++)
        b.push(sub(sub(coffs[i], mul(p, b[sub(i, 1)])), mul(q, b[sub(i, 1)])));
    const c = [1, sub(b[1], p)];
    for (let i = 2; i < coffs.length; i++)
        c.push(sub(sub(b[i], mul(p, c[sub(i, 1)])), mul(q, c[sub(i, 2)])));
    let counter = 0;
    while (true) {
        counter++;
        ...
        b[1] = sub(coffs[1], p);
        for (let i = 2; i < coffs.length; i++)
            b[i] = sub(sub(coffs[i], mul(p, b[sub(i, 1)])), mul(q, b[sub(i, 2)]));
        c[1] = sub(b[1], p);
        for (let i = 2; i < coffs.length; i++)
            c[i] = sub(sub(b[i], mul(p, c[sub(i, 1)])), mul(q, c[sub(i, 2)]));
    ...
    const new_coffs = [];
    for (let i = 0; i < sub(b.length, 2); i++)
        new_coffs.push(b[i]);
