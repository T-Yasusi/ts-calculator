...
export default function gaussianElimination(mat) {
    if (mat.cols !== mat.rows)
        throw new Error("!!! Gaussian Elimination  not Square Matrix !!!");
    ...
    for (let i = 0; i < mat.cols; i++) {
        ...
        const a = copied[i][i];
        if (abs(a) < THRESHOLD)
            throw new Error('Gaussian Elimination Matrix is singular');
