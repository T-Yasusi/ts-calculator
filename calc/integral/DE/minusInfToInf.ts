...
export default function minusInfToInf(f: ((x: number) => number) | ((x: Complex) => Complex), threshold: number = 1.0e-8) {
  ...
  if (isReal) {
    const phi = (t: number): number => sinh(mul(div(Math.PI, 2), sinh(t))) as number;
    const phiPrime = (t: number): number => mul(mul(div(Math.PI, 2), cosh(t)), cosh(mul(div(Math.PI, 2), sinh(t)))) as number;
    const g = (t: number): number => mul((f as (x: number) => number)(phi(t)), phiPrime(t));
    return converge(g, threshold);
  } else {
    const phi = (t: Complex): Complex => sinh(mul(div(Math.PI, 2), sinh(t))) as Complex;
    const phiPrime = (t: Complex): Complex => mul(mul(div(Math.PI, 2), cosh(t)), cosh(mul(div(Math.PI, 2), sinh(t)))) as Complex;
